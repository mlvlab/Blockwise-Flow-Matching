...
def main(args):
    ...
    # create vae
    vae = AutoencoderKL.from_pretrained(f"stabilityai/sd-vae-ft-mse", torch_dtype=weight_dtype).to(device)
    requires_grad(ema, False)
    if accelerator.is_main_process:
        ...
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(model):
                ...
                if args.model.network_config.finetune:
                    ...
                    denoise_loss, proj_loss = scheduler.compute_loss3(model, x, model_kwargs, zs=zs[0], segment_idx=segment_idx)
            ...
            if accelerator.sync_gradients:
                ...
                if (global_step == 1 or (global_step % args.evaluation.sampling_steps == 0 and global_step > 0)):
                    with accelerator.autocast():
                        samples = scheduler.sample_ode_frn(
                            ema,
                            xT,
                            ys,
