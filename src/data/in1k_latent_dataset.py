...
class IN1kLatentDataset_onesample(Dataset):
    def __init__(self, root_dir, target_len=256, random='random'):
        super().__init__()
        self.RandomHorizontalFlipProb = 0.0
        self.root_dir = root_dir
        self.target_len = target_len
        self.random = random
        self.files = []
        files_1 = os.listdir(osp.join(root_dir, f'greater_than_{target_len}_crop'))
        self.files.extend([
            [osp.join(root_dir, f'greater_than_{target_len}_crop', file)] for file in files_1
        ])
...
class INLatentLoader_onesample():
    def __init__(self, train):
        super().__init__()
        ...
        self.train_dataset = IN1kLatentDataset_onesample(
            self.train_config.data_path, self.train_config.target_len, self.train_config.random
        )
